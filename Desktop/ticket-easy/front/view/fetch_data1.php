...
if (isset($_POST["action"])) {
    $query = $query = "SELECT s.*, cs.* FROM movie s 
    INNER JOIN moviecategory cs ON s.idcategory = cs.idcategory";


    if (isset($_POST["categoryName"])) {
        $category_filter = implode("','", $_POST["categoryName"]);
        $query .= "
	  where categoryName IN('" . $category_filter . "')
	 ";
    }

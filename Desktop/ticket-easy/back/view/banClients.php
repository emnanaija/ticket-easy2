...
// Ban selected clients
if (isset($_POST['banSelectedClients'])) {
    $selectedClients = $_POST['selectedClients'];
    foreach ($selectedClients as $clientId) {
        $clientC->banClient($clientId);
    }
}

// Get banned clients
$bannedClients = $clientC->getBannedClients();
?>
...
<table>
  ...
  <tbody>
    <?php foreach ($bannedClients as $client) { ?>
      <tr>
        ...
        <td><a href='unbanClient.php?id=<?= $client['idClient'] ?>'>Unban</a></td>
      </tr>
    <?php } ?>
  </tbody>
</table>

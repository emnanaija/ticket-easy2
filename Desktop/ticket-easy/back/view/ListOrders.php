...
//$orderC = new OrderC();
//$list = $orderC->listOrders();

$calc = new OrderC();
$list1 = $calc->listOrders();
$date = $calc->showDate();
$Total = 0;
foreach ($list1 as $l) {

    if ($l['orderDate'] == $date['orderDate']) {
        $Total = $Total + $l['priceTotal'];
    }
}
    ...
    <div id="main-wrapper">
        ...
        ***********************************-->
        ...
        <!--**********************************
            Header start
        ***********************************-->
<style>button, [type="button"], [type="reset"], [type="submit"] {
    /* -webkit-appearance: button; */
    color: #fff;
    background-color: #ff0000;
    border-color: #fff;
...
.header-left input {
    ...
    border-width: 85px -1px 18px 8px;
    font-weight: 200;
}
                            ...
                            <div class="search_bar dropdown show">
                                <div class="dropdown-menu p-0 m-0 show">
                                    <form method="GET" action="">
                                        <input class="form-control" type="text" placeholder="Search by date" name="search_date">
...
// Check if a search date was submitted
if (isset($_GET['search_date']) && !empty($_GET['search_date'])) {
    $search_date = $_GET['search_date'];
    $pdo = config::getConnexion();
    $stmt = $pdo->prepare("SELECT * FROM oorder WHERE orderDate = :search_date");
    $stmt->execute(['search_date' => $search_date]);
    $filtered_list = $stmt->fetchAll();
}
else {
    // If no search date was submitted, retrieve all orders
    $orderC = new OrderC();
$filtered_list = $orderC->listOrders();
}

// Iterate through all orders and display them in a table
foreach ($filtered_list as $order) {
                                ?>
                                    <tr>
                                        <td><?= $order['idOrder']; ?></td>
                                        ...
                                        <?php if ($order['orderState'] == 'Ready') { ?>
                                            <td><span class="badge light badge-success"><?= $order['orderState']; ?></span></td>
                                        <?php } else { ?>
                                            <td><span class="badge light badge-danger"><?= $order['orderState']; ?></span></td>
                                        <?php } ?>


                                        <td><?= $order['priceTotal']; ?> DT</td>

                                        <td align="center">
                                            <form method="GET" action="detailUser.php">

                                                <input type="submit" name="detail" value="User Details" class="btn btn-primary">
                                                <input type="hidden" value=<?PHP echo $order['idClient']; ?> name="idOrder" id="idOrder">
                                            </form>
                                        </td>
                                        ...
                                        <td align="center">
                                            <form method="GET" action="updateOrder.php">
                                                ...
                                                <input type="hidden" value="<?PHP echo $order['idOrder']; ?>" name="idOrder" id="idOrder">
                                        ...
                                        <?php
											$pdo = config::getConnexion();
											$id = $order['idOrder'];
											// Check if there are any snacks associated with this category
											$sql = "SELECT COUNT(*) as count FROM payment WHERE idOrder = $id";
											...
											if ($result['count'] > 0) {
												// If there are snacks associated with this category, show an alert box
												echo '<a href="#" class="text-danger" onclick="showAlert(this);"><i class="ti-trash"></i></a>';
											} else {
												// If there are no snacks associated with this category, show the delete button
												echo '<a href="deleteOrder.php?idOrder=' . $order['idOrder'] . '" class="text-danger" data-bs-original-title="Delete" data-bs-toggle="tooltip"><i class="ti-trash"></i></a>';
											}

											?>
										</td>
										<script>
											function showAlert(trashIcon) {
												var alertBox = document.createElement("div");
												alertBox.id = "alert-box";
												alertBox.innerHTML = "Cannot delete this Order as it has Payments associated with it";
												...
												alertBox.style.transform = "translateX(-60%)";
												alertBox.style.zIndex = "9999";
												document.body.appendChild(alertBox);

												// Hide alert box when mouse leaves trash icon

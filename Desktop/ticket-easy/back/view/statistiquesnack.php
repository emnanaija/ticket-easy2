...
$total_lines=0;
foreach($list as $l){
$qt=$qt+$l['quantitySnack'];

}
$db = config::getConnexion();
$total_lines=$db->query('SELECT * FROM snack')->rowCount();
?>
							...
							<div class="card-body orders-summary">
                                ...
                                <div class="col-sm-4 mb-4">
										<div class="border px-3 py-3 rounded-xl">
											<h2 class="fs-32 font-w600 counter"><?php echo $qt ?></h2>
											<p class="fs-16 mb-0">Total Quantity</p>
										</div>
									</div>
                                <div class="chartCard">
      <div class="chartBox">
    ...
    <?php
                            ...
                            try{
                                ...
                                while($row = $result->fetch()){
                                    $menuArray[] = $row["snackName"];
                                    $quantityArray[] = $row["quantitySnack"];
                                }
                                unset($result);
                                }else {
                                echo 'No Result';

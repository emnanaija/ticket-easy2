...
-->
        ...
        ***********************************-->
        ...
        <h2>
            <a href="addreclamation.php" class="btn btn-primary light ">Add Reclamation</a>
        </h2>
            </center>
  ...
  <option value="1">Trier par Prénom</option>
  <option value="2">Trier par Nom</option>
  <option value="3">Trier par Adresse</option>
  <option value="4">Trier par Date de réclamation</option>
  <option value="5">Trier par Réclamation</option>
...
document.querySelector('#sortSelect').addEventListener('change', (event) => {
  ...
  <thead>
    <tr>
      ...
      <th>Etat</th>
        ...
        foreach ($list as $reclamation) {
           ...
           // $sql = "SELECT client.numClient from client,reclamation where client.address = reclamation.address";
        ...
        ?>
            ...
            <tr>
                <?php $id=$reclamation['idRec']; ?>
                <?php $mail=$reclamation['address']; ?>
                <?php $reclam=$reclamation['texte']; ?>

                <td><?= $reclamation['idRec']; ?></td>
                <td><?= $reclamation['lastName']; ?></td>
                <td><?= $reclamation['firstName']; ?></td>
                <td><?= $reclamation['address']; ?></td>
                <td><?= $reclamation['dor']; ?></td>
    ...
    <form method="POST" action="updateReclamation.php">
        <input class="btn btn-primary light" type="submit" name="update" value="Update">
        <input class="btn btn-primary light" type="hidden" value=<?PHP echo $reclamation['idRec']; ?> name="idRec">
    </form>
</td>

<td>
    <form method="POST" action="deleteReclamation.php">
        <input type="hidden" name="idRec" value="<?php echo $reclamation['idRec']; ?>">
...
<td>
  ...
  <?php echo('<form method="POST" action="testmail.php?id='.$id.'&email='.$mail.'&reclamation='.$reclam.' ">') ?>
    <input type="hidden" name="address" value="<?php echo $reclamation['address']; ?>">
    <button class="btn btn-primary light" id="repondre-button">Repondre</button>
  </form>
...
<script>
    ...
    statusSelect.addEventListener("change", function() {
        ...
        if (selectedValue === "terminee") {
            alert("La réclamation est terminée !"); // Afficher une alerte
            deleteButton.disabled = false; // Désactiver le bouton de suppression
        } else if (selectedValue === "en_attente") {
            deleteButton.disabled = true; // Activer le bouton de suppression
        }
    });
</script>

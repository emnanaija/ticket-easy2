...
$productC = new snackC();
if (
	isset($_POST["snackName"]) &&
	isset($_POST["priceSnack"]) &&
	isset($_POST["quantitySnack"]) &&
	isset($_FILES["imageSnack"]) &&
	isset($_POST["idcategoriesnack"])

) {
	$error = "";
	$target_dir = "./snacks/";
	$target_file = $target_dir . basename($_FILES["imageSnack"]["name"]);
	$uploadOk = 1;
	$imageFileType = strtolower(pathinfo($target_file, PATHINFO_EXTENSION));

	// Check if image file is a actual image or fake image
	if (isset($_POST["submit"])) {
		$check = getimagesize($_FILES["imageSnack"]["tmp_name"]);
		if ($check !== false) {
	...
	// Check file size
	if ($_FILES["imageProd"]["size"] > 500000) {
	...
	// if everything is ok, try to upload file
		...
		if (move_uploaded_file($_FILES["imageSnack"]["tmp_name"], $target_file)) {
			$error = "";
		}
	}
	if ($error == "")
		...
		if (
			!empty($_POST['snackName']) &&
			!empty($_POST["priceSnack"]) &&
			!empty($_POST["quantitySnack"]) &&
			!empty($_FILES["imageSnack"]) &&
			!empty($_POST["idcategoriesnack"])
			...
			$product = new Snack(
				...
				$_POST['priceSnack'],
				$_POST['quantitySnack'],
				$_FILES['imageSnack']['name'],
				$_POST['idcategoriesnack']
			);
			$productC->updatesnack($product, $_POST["idSnack"]);
						...
						<div class="card">
							...
							<div class="card-body">
								<div class="basic-form">
									...
									<?php if (isset($_POST['idSnack'])) {
										$product = $productC->showSnack($_POST['idSnack']);
									?>
										<form action="" method="POST" id="OrderForm" id="OrderForm" enctype="multipart/form-data">
											...
											<div class="form-row">
												...
												<div class="form-group col-md-6">
													...
													<input type="number" class="form-control" readonly id="idSnack" name="idSnack" value="<?php echo $product['idSnack']; ?>">
												</div>


												<div class="form-group col-md-6">
													<label for="productName">Add the name of the Snack</label>
													<input type="text" class="form-control" placeholder="name" name="snackName" id="snackName" value="<?php echo $product['snackName']; ?>">
												</div>

												<div class="form-group col-md-6">
													<label for="priceProd">Add the price of the Snack</label>
													<input type="number" class="form-control" placeholder="price" name="priceSnack" id="priceSnack" value="<?php echo $product['priceSnack']; ?>">
												</div>

												<div class="form-group col-md-6">
													<label for="quantityProd">Add the quantity of the Snack</label>
													<input type="number" class="form-control" placeholder="quantity" name="quantitySnack" id="quantitySnack" value="<?php echo $product['quantitySnack']; ?>">
												</div>
												<div class="form-group col-md-6">
													<label for="imageProd">Add the image of the Sanck</label>
													<input type="file" class="form-control" placeholder="image" name="imageSnack" id="imageSnack" value="./snacks/<?php echo $product['imageSnack']; ?> ">
												</div>
													...
													<div id="categorySelect">
														<?php foreach ($result as $row) { ?>
															<div class="form-check">
																<input class="form-check-input categorySelect" type="radio" name="idcategoriesnack" id="idcategoriesnack" value="<?php echo $row['idcategoriesnack']; ?>">
																<label class="form-check-label" for="<?php echo $row['categoriesnack']; ?>">
																	<?php echo $row['categoriesnack']; ?>
																</label>
															</div>
														<?php } ?>
													</div>
    ...
    ***********************************-->
	...
	<script>
		...
		form.addEventListener('submit', (event) => {
			...
			const snackName = document.querySelector('#snackName');
			const priceSnack = document.querySelector('#priceSnack');
			const quantitySnack = document.querySelector('#quantitySnack');
			const idcategoriesnack = document.querySelectorAll('.categorySelect:checked');

			let isValid = true;

			if (snackName.value === '' || !/^[a-zA-Z]+$/.test(snackName.value)) {
				Swal.fire({
					icon: 'error',
					title: 'Oops...',
					text: 'Please enter a valid snack Name (only letters)',
				});
				isValid = false;
			}
			if (priceSnack.value === '' || isNaN(priceSnack.value) || priceSnack.value < 0) {
				Swal.fire({
					icon: 'error',
					title: 'Oops...',
					text: 'Please enter a valid price Snack (only Numbers)',
				});
				isValid = false;
			}

			if (quantitySnack.value === '' || isNaN(quantitySnack.value) || quantitySnack.value < 0) {
				Swal.fire({
					icon: 'error',
					title: 'Oops...',
					text: 'Please enter a valid quantity Snack (numeric value greater than or equal to 0)',
				});
				isValid = false;
			}

			if (idcategoriesnack.length === 0) {
			Swal.fire({
				icon: 'error',
				title: 'Oops...',
